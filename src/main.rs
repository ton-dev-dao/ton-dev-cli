...
use ton_dev_client::abi::{CallSet, ParamsOfEncodeMessageBody, TokenValueToStackItem};
...
async fn encode_command(matches: &ArgMatches<'_>, config: &Config) -> Result<(), String> {
    ...
    let x = serde_json::from_str::<ton_dev_abi::Param>(str_abi.as_str()).map_err(|e| e.to_string())?;
    ...
    let mut opt_cell: Option<ton_dev_block::Cell> = None;
    ...
    let msg_bytes = ton_dev_block::write_boc(&opt_cell.unwrap())
